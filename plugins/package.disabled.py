...
# of A,B and one of C since C doesn't *always* appear with A,B. It's
# naive, but it works.
...
# Otherwise set to False to only package on deploy. You may have to clean
# up autogen files manually:
PACKAGE_LOCALLY_DEBUG = False
# whether to fetch and package remotely hosted files
INCLUDE_REMOTE_ASSETS = True
# otherwise all package filenames will be a concatenation of the filenames
# within
MINIFY_FILENAMES = False
...
def _getAssetFrom(tag, site, save=False):
    ...
    if url:
        ...
        if url.startswith(_staticPath(site)):
            # change 'static/js/foo' to
            # '/full/absolute/static/.build/static/js/foo'
...
def _replaceHTMLWithPackaged(html, replace_map, path, site):
    ...
    for tag in _getLinks(soup) + _getScripts(soup):
        ...
        path_to_static = os.path.relpath(
            _staticPath(site, includeBuild=True), _getDir(path))
        new_url = os.path.join(path_to_static, replace_map[asset])
        if new_url in replaced:
            # remove HTML node; this was already covered by another node with
            # same package
            tag.extract()
        else:
...
def analyzeAndPackageAssets(site):
    sys.stdout.write('Analyzing %d gathered assets across %d pages...' %
                     (len(list(itertools.chain.from_iterable(assets))),
                      len(assets))
                     )
    sys.stdout.flush()
    replace_map = {}
    ...
    for i, package in enumerate(packages):
        ...
        if len(package) <= 1 and (no_local or not COMPRESS_PACKAGES):
            ...
            # also silly for a local file to be packaged with itself if we
            # won't be compressing it
            continue
...
def postBuildPage(site, path):
    ...
    if PACKAGE_JS:
        assets.append(
            map(lambda x: _getAssetFrom(x, site, save=True), _getScripts(soup)))
    if PACKAGE_CSS:
        assets.append(
            map(lambda x: _getAssetFrom(x, site, save=True), _getLinks(soup)))
